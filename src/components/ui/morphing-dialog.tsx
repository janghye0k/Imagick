...
import React, { useCallback, useContext, useEffect, useEffectEvent, useId, useMemo, useRef, useState } from 'react';
...
function MorphingDialogContent({ children, className, style }: MorphingDialogContentProps) {
  ...
  const handleSetFocusableElements = useEffectEvent((isOpen: boolean, triggerElement?: HTMLElement | null) => {
    if (isOpen) {
  ...
  useEffect(() => {
    handleSetFocusableElements(isOpen, triggerRef.current);
  }, [isOpen, triggerRef]);
...
function MorphingDialogContainer({ children }: MorphingDialogContainerProps) {
  ...
  const effectSetMounted = useEffectEvent(setMounted);
  useEffect(() => {
    effectSetMounted(true);
    return () => effectSetMounted(false);
  }, []);
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 h-full w-full bg-white/40 backdrop-blur-xs z-49 dark:bg-black/40"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
