...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'gap-1.5 px-2.5 py-1.5 text-xs shadow-xl grid min-w-[8rem] items-start rounded-lg border border-border/50 bg-background',
        className
      )}
    >
...
function ChartLegendContent({
  ...
  return (
    <div className={cn('gap-4 flex items-center justify-center', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload
        .filter((item) => item.type !== 'none')
        .map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn('gap-1.5 [&>svg]:h-3 [&>svg]:w-3 flex items-center [&>svg]:text-muted-foreground')}
            >
